...
fn reference_test(file: &str) {
    reference_test_with_options(file, ai_image_webp::WebPDecodeOptions::default(), None);
}

fn reference_test_with_options(
    file: &str,
    options: ai_image_webp::WebPDecodeOptions,
    custom_reference_file: Option<&str>,
) {
    // Prepare WebP decoder
    let contents = std::fs::read(format!("tests/images/{file}.webp")).unwrap();
    let mut decoder =
        ai_image_webp::WebPDecoder::new_with_options(Cursor::new(contents), options).unwrap();
    ...
    ($basename:expr, $name:expr) => {
        paste::paste! {
            ...
            fn [<reftest_nofancy_ $basename _ $name>]() {
                let mut options = ai_image_webp::WebPDecodeOptions::default();
                options.lossy_upsampling = ai_image_webp::UpsamplingMethod::Simple;
